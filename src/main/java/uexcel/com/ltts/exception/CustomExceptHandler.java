...
public class CustomExceptHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<String> handleCustomExcept(CustomException ex) {
        ...
        return switch (ex.getCode()) {
            ...
            case "402" -> ResponseEntity.status(HttpStatus.PAYMENT_REQUIRED).body(ex.getMessage());
            case "401" -> ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(ex.getMessage());
